...
public class LogPlugin extends CQPlugin {
  ...
  public int onPrivateMessage(CoolQ cq, CQPrivateMessageEvent event) {
    MyLog myLog = MyLog.builder()
        ...
        .build();
    logger.info("EVENT self:{} type:{} userId:{} content:{}", myLog.getSelfId(), myLog.getType(),
        myLog.getUserId(), myLog.getContent());
  ...
  public int onGroupDecreaseNotice(CoolQ cq, CQGroupDecreaseNoticeEvent event) {
    MyLog myLog = MyLog.builder()
        .selfId(cq.getSelfId())
        .type(
            "event_" + event.getPostType() + "_" + event.getNoticeType() + "_" + event.getSubType())
        .groupId(event.getGroupId())
        .userId(event.getUserId())
  ...
  public int onGroupRequest(CoolQ cq, CQGroupRequestEvent event) {
    MyLog myLog = MyLog.builder()
        .selfId(cq.getSelfId())
        .type("event_" + event.getPostType() + "_" + event.getRequestType() + "_" + event
            .getSubType())
        .groupId(event.getGroupId())
        .userId(event.getUserId())
        .content(event.getComment())
